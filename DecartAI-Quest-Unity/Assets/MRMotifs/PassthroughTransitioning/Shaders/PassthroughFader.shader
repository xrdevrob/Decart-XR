...
Shader "Meta/PassthroughFader"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // ✅ Use premultiplied alpha blending for passthrough compositing
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float distance_from_center = length(i.uv - center);

                // Choose fade direction
                if (_FadeDirection == 0)
                {
                ...
                {
                    fadeFactor = 1.0f - i.uv.x;  // left-to-right
                }
                else if (_FadeDirection == 2)
                {
                    fadeFactor = 1.0f - i.uv.y;  // bottom-to-top
                }
                else if (_FadeDirection == 3)
                {
                    fadeFactor = 1.0f - distance_from_center; // radial
                }
                ...
                // ✅ Invert so higher dissolve = more passthrough
                col.a = 1.0 - mask;

                // ✅ Premultiply color for proper passthrough compositor blending
                col.rgb *= col.a;
